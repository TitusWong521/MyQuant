...
def get_MA(coin, gate_query):
    ...
    log(coin, '%.6f, %.6f, %.6f, %.6f' % (cur_short, last_short, cur_long, last_long))
...
def buy(coin, orders, total_pool, gate_trade):
    ...
    if len(sums) < 1:
        ...
        content = content + log(coin, '以【%.6f】的价格买入了【%.6f】个，总价值为【%.6f】' % (float(orders['asks'][0][0]), buy_count, total_pool))
        ...
        for index in range(len(sums)):
            ...
            content = content + log(coin, '以【%.6f】的价格买入了【%.6f】个，总价值为【%.6f】' % (float(orders['asks'][index][0]), float(orders['asks'][index][1]), float(orders['asks'][index][0]) * float(orders['asks'][index][1])))
        ...
        content = content + log(coin, '以【%.6f】的价格买入了【%.6f】个，总价值为【%.6f】' % (float(orders['asks'][len(sums)][0]), buy_count, rest_pool))
        content = content + log(coin, '以上总共买入了价值【%s】的【%s】' % (total_pool, coin))
...
def sell(coin, orders, total_count, gate_trade):
    ...
    if len(sums) < 1:
        ...
        content = content + log(coin, '以【%.6f】的价格卖出了【%.6f】个，总价值为【%.6f】' % (float(orders['bids'][0][0]), total_count, float(orders['bids'][0][0]) * total_count))
        ...
        for index in range(len(sums)):
            ...
            content = content + log(coin, '以【%.6f】的价格卖出了【%.6f】个，总价值为【%.6f】' % (float(orders['bids'][index][0]), float(orders['bids'][index][1]), float(orders['bids'][index][0]) * float(orders['bids'][index][1])))
        rest_count = total_count - sums[-1]
        # gate_trade.sell(coin, float(orders['bids'][len(sums)][0], rest_count))
        content = content + log(coin, '以【%.6f】的价格卖出了【%.6f】个，总价值为【%.6f】' % (float(orders['bids'][len(sums)][0]), rest_count, rest_count * float(orders['bids'][len(sums)][0])))
        content = content + log(coin, '以上总共卖出了价值【%s】的【%s】' % (total_pool + rest_count * float(orders['bids'][len(sums)][0]), coin))
...
def start_script(coin, total_pool, total_count):
    ...
    log(coin, '开始测试...初始资金池为 %d' % total_pool)
    buy_sell_flag = True
        ...
        try:
            ...
            if signal == 1:
                ...
                if buy_sell_flag:
                    ...
                    log(coin, '取消买入【%s】动作，因为最近已经买入过' % coin + '\n当前持仓为: 【现金】 %.6f, 【%s】 %.6f 个' % (total_pool, coin, total_count))
            time.sleep(60)
        except Exception as ex:
            log(coin, 'ERROR\t' + str(ex.args))
...
def login_wechat():
    ...
    itchat.send('test11111', 'filehelper')
...
if __name__ == '__main__':
    ...
    p = Pool(1)
    p.apply_async(start_script, args=(lst_coin[0], total_pool, total_count))
    p.close()
    # p.join()
    # login_wechat()